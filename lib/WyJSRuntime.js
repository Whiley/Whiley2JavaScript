...
var WyJS = {}; // main object

// Augmenting Function object with method function so it
// doesn't have to use prototype keyword in method declarations.
...
// Performs the AND bitwise operator.
WyJS.Byte.method('and', function(value){
	var result = this.val & value.val;
	return new WyJS.Byte(result);
});

//Performs the OR bitwise operator.
WyJS.Byte.method('or', function(value){
	var result = this.val | value.val;
	return new WyJS.Byte(result);
});

// Performs the XOR bitwise operator.
WyJS.Byte.method('xor', function(value){
	var result = this.val ^ value.val;
	return new WyJS.Byte(result);
});

// Performs the SHL bitwise operator.
WyJS.Byte.method('shl', function(value){
	var result = this.val << value.val;
	return new WyJS.Byte(result);
});

// Performs the SRL bitwise operator.
WyJS.Byte.method('srl', function(value){
	var result = this.val >>> value.val;
	return new WyJS.Byte(result);
});

// Performs the NOT bitwise operator.
WyJS.Byte.method('invert', function(){
	var result = ~this.val;
	 var temp = result;
	  result = result & (0xff);
	return new WyJS.Byte(result);
...
WyJS.Byte.method('equals', function(value){
	return WyJS.equals(this.val, value.val);
});
...
// Performs addition operator in Real type.
WyJS.Real.method('add', function (other) {
  return new WyJS.Real(this.val + other.val);
});

// Performs subtraction operator in Real type.
WyJS.Real.method('sub', function(other) {
  return new WyJS.Real(this.val - other.val);
});

// Performs multiplication operator in Real type.
WyJS.Real.method('mul', function(other) {
  return new WyJS.Real(this.val * other.val);
});

// Performs division operator in Real type.
WyJS.Real.method('div', function(other) {
  return new WyJS.Real(this.val / other.val);
});

// Performs remainder operator in Real type.
WyJS.Real.method('rem', function(other) {
  return new WyJS.Real(this.val % other.val);
});

// Performs negation operator in Real type.
...
// Performs addition operator in Integer type.
...
// Performs subtraction operator in Integer type.
...
// Performs multiplication operator in Integer type.
...
// Performs division operator in Integer type.
...
// Performs remainder operator in Integer type.
...
// Performs negation operator in Integer type.
WyJS.Integer.method('neg', function(){
  return new WyJS.Integer(-this.val);
});

// Performs casting operator into Integer type.
...
//C hecks if a given field exists
...
// Checks deep equality for each element of the Record type.
...
// Checks deep equality for each element of the Array type.
...
// Creates a sublist from the index given to the other.
...
// 		lhs - left operator to be compared
//		rhs - right operator to be compared
// 		isEqual - boolean value for equality
...
// checks if an item is contained in a list
WyJS.inList = function(item, list){
 for (var i = 0; i > list.length; i++) {
    if(WyJS.equals(item, list.getValue(i))){
      return true;
    }
  }
  return false;
...
WyJS.is = function(obj, type) {
  ...
  else if(type instanceof WyJS.Type.Byte) {
	  if(obj instanceof WyJS.Byte) {
		  return true;
	  }
	  return false;
  }

  else if (type instanceof WyJS.Type.Bool) {
