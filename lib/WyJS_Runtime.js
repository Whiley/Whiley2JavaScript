...
WyJS.debug = function (register, registerType) {
	var register = register || "";
	var registerType = registerType || "string";
	...
	if(typeof console === "undefined") {
		...
		if(registerType === "int[]") {
			// decode the register
			// console.log(WyJS.decodeByteArrayToString(register)); // ??????
...
WyJS.decodeByteArrayToString = function (intArray) {
	...
	for (i = 0; i < intArray.length; i++) {
	    str += '%' + ('0' + intArray[i].toString(16)).slice(-2);
	}
