...
WyJS.Byte = function (i) {
  ...
  print("NEW BYTE :: "+this.val);
  this.type = new WyJS.Type.Byte();
}

WyJS.Byte.method('and', function(value){
	print("and "+this.val+" & "+value.val+ " = "+(this.val & value.val));
...
WyJS.Byte.method('invert', function(){
	var result = ~this.val;
	print("got in byte invert");
	return new WyJS.Byte(result);
});

WyJS.Byte.method('equals', function(value){
	if(value.type === undefined || value.type !== WyJS.Type.Byte) {
		return false;
	}
...
WyJS.equals = function(lhs, rhs, isEqual) {
  print("equals: lhs: "+lhs.val+" rhs: "+rhs.val+ " isEquals: "+isEqual);
	var left = lhs;
  if (left.type !== undefined) {
      if (!WyJS.is(rhs, left.type)) {
          print("!WyJS.is(rhs, left.type)"+false);
          return false;
      }
  }
  ...
  if (rhs.type !== undefined) {
      if (!WyJS.is(left, rhs.type)) {
    	  print("!WyJS.is(left, rhs.type)"+false);
    	  return false;
      }
  }
  if (left.val !== undefined) {
      ...
      if (isEqual) {
    	  print("left.equals(rhs)"+left.equals(rhs));
...
WyJS.gt = function(lhs, rhs, isEqual) {
 print("gt: lhs: "+lhs.val+" rhs: "+rhs.val+" isEqual: "+isEqual);
  var left = lhs;
  if (left.val !== undefined) {
      left = left.val;
    } else {
    	print("undefined left");
    }
  var right = rhs;
  if (right.val !== undefined) {
      right = right.val;
    }  else {
    	print("undefined right");
    }
  if (isEqual) {
	  print(left >= right);
      return (left >= right);
