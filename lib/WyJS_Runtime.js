...
WyJS.Byte = function (i) {
  ...
  if (i.pure !== undefined) {
	    this.pure = i.val;
	    if(this.pure.length < 8) { // checks the length of this.pure
	    	var pureArray = this.pure.toString().split("");
	    	pureArray.reverse();
	    	for(var j = this.pure.length; j < 8; j++) {
	    		pureArray.push("0"); // fill with left zeros
	    	}

	    	pureArray = pureArray.reverse();
	    	pureArray = pureArray.join("");
	    	this.pure = pureArray;
	    }
	    print("pure "+this.pure);
	  }
	  else {
	    this.pure = i.slice(0, -1); // removes b
	    if(this.pure.length < 8) { // checks the length of this.pure
	    	var pureArray = this.pure.toString().split("");
	    	pureArray.reverse();
	    	for(var j = this.pure.length; j < 8; j++) {
...
WyJS.Byte.method('and', function(value){
	if(value.pure === undefined) {
		value.pure = this.pure;
	}
	print("and " + (this.pure & value.pure)+"b" + " this.pure: "+this.pure+" value.pure: "+value.pure);
	return new WyJS.Byte((this.pure & value.pure)+"b");
});

WyJS.Byte.method('or', function(value){
	print("or "+(this.pure | value.pure)+"b");
	return new WyJS.Byte((this.pure | value.pure)+"b");
});

WyJS.Byte.method('xor', function(value){
	print("xor "+(this.pure ^ value.pure)+"b");
	return new WyJS.Byte((this.pure ^ value.pure)+"b");
});

WyJS.Byte.method('sll', function(value){
	var result = (this.pure << value).toString(2);
	print("sll "+ result+"b" +" this.pure: " + this.parseToInt(this.pure) + " value: " + value.val);
	return new WyJS.Byte(result+"b");
});

WyJS.Byte.method('srl', function(value){
	print("srl "+(WyJS.Byte.parseToInt(this.pure) >>> value.val)+"b");
	return new WyJS.Byte((this.pure >>> value.val)+"b");
});
...
WyJS.List.method('getValue', function(index) {
  var idx = index;
...
WyJS.List.method('setValue', function(index, value) {
  var idx = index;
  if (index.val !== undefined) {
      idx = index.val;
    }
...
WyJS.List.method('clone', function(type) {
...
WyJS.List.method('equals', function(other) {
...
WyJS.List.method('sublist', function(from, to){
  var data = [];
  var index = 0;
...
WyJS.encodeStringToByteArray = function(str) {
    ...
    var finalByteArray = new WyJS.List(intArray, new WyJS.Type.List(new WyJS.Type.Int()));
    return intArray;
};
...
WyJS.Type.List.method('subtype', function(superType) {
