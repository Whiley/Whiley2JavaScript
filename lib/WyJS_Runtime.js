...
WyJS.debug = function (register) {
	...
	if(typeof console === "undefined") {
		...
		// decode the register in case it's a list of integers
		if(WyJS.is(register.type, WyJS.Type.List)) {
			// check the type of the list
			var elementsOfList = [];
			var currentValue, isIntegerList = true;
			elementsOfList = register.list;
			for(i = 0, len = elementsOfList.length; i < len; i++) {
				currentValue = elementsOfList.getValue(i);
				if(!WyJS.is(currentValue, WyJS.Type.Int)) {
					isIntegerList = false;
				}
			}

			if(isIntegerList) {
				console.debug(WyJS.decodeByteArrayToString(elementsOfList));
			}
		} else {
			// if it's a string already, no need for decoding
